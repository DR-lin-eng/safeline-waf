...
function _M.get_pow_script()
    local html = {
        ...
        '        setStatus("正在获取挑战…");',
        '        if (!currentToken) {',
        '          throw new Error("missing_verification_token");',
        '        }',
        '        return fetch("/pow/challenge?token=" + encodeURIComponent(currentToken), {',
        '          method: "GET",',
        ...
        '        setStatus("验证中…");',
        '        if (!currentToken) {',
        '          return Promise.reject(new Error("missing_verification_token"));',
        '        }',
        '        return fetch("/pow/verify", {',
        '          method: "POST",',
        '          credentials: "same-origin",',
        '          headers: { "Content-Type": "application/json", "Accept": "application/json" },',
        '          body: JSON.stringify({ token: currentToken, challenge_id: challengeId, nonce: nonce })',
        '        }).then(function(res){',
        '          return safeJson(res).then(function(data){',
