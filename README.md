...
- **新安装**：仅适用于干净环境。若检测到已有安装痕迹（如 `.env` 或现有源码目录），脚本会拒绝继续，并提示改选“升级”或“覆盖安装”。
- **升级**：保留现有 `.env`、`JWT_SECRET`、`REDIS_PASSWORD`、`ADMIN_USERNAME` 和 `ADMIN_PASSWORD_HASH`。升级不会重置管理员密码；安装完成提示也会明确说明“密码未重置，请继续使用升级前的密码登录”。
- **覆盖安装**：重新准备运行时环境并重建管理员凭据，但不会清空 `config/sites`、证书等现有数据目录。旧管理员密码会失效。
- **退出**：直接退出安装脚本。

首次安装与覆盖安装都会要求设置管理员用户名和密码，并生成 `.env` 中的安全密钥。
如果是非交互安装，可通过环境变量预置管理员凭据：
...
- **升级模式**：若现有 `.env` 完整，则继续保留原有管理员凭据，不会生成新密码。
- **首次安装 / 覆盖安装**：脚本会生成一次性临时密码，并仅在安装完成时显示一次。
...
# 准备环境变量
cp .env.example .env
# 按实际环境填写 REDIS_PASSWORD、JWT_SECRET、ADMIN_PASSWORD_HASH 等关键配置
```
...
说明：验证结果始终通过安全 Cookie 传递；当前可配置的绑定维度为 IP 与 User-Agent。
...
1. 准备 SSL 证书文件
2. 通过管理界面上传证书内容，或将证书放入 `config/certs/` 目录
3. 在管理界面中为站点启用 HTTPS
4. 保存站点后系统会自动热发布，无需手工重启 nginx
4. 提供证书文件路径
5. 重启服务以应用更改
...
- `scripts/doctor.sh`：检查 Compose、Redis、Nginx 语法与热重载链路
- `scripts/security_smoke.sh`：端到端验证站点发布、HTTP/HTTPS、自签证书、JS/F12 注入、限速挑战与 SQLi/XSS 拦截
- `scripts/llm_direct_test.sh`：不依赖运行中的后端容器，直接用本地 `llm_worker.js` 测试 LLM 接口兼容性
...
若要直接测试 OpenAI Responses 兼容接口：

```bash
API_KEY='your-llm-key' \
API_ENDPOINT='https://api.openai.com/v1/responses' \
MODEL='gpt-4.1-mini' \
./scripts/llm_direct_test.sh
```
...
所有管理 API 请求都需要通过 Bearer JWT 进行认证。默认由前端通过 `/safeline-admin-api` 反向代理访问后端 `/api/*`：
...
- `GET /api/llm/config` - 获取 LLM 审计配置
- `PUT /api/llm/config` - 更新 LLM 审计配置
- `POST /api/llm/test` - 测试当前 LLM 配置连通性
